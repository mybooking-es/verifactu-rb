module Verifactu
  ...
  class RegistroAltaXmlBuilder
    ...
    def self.agregar_registro_alta(xml_document_root, registro)
      ...
      # FechaOperacion
      if registro.fecha_operacion and !registro.fecha_operacion.to_s.empty?
        fecha_operacion_element = Nokogiri::XML::Node.new('sum1:FechaOperacion', xml_document_root)
        fecha_operacion_element.content = registro.fecha_operacion
      ...
      # Agregar destinatarios — omitido en F2/R5 (factura simplificada sin
      # destinatario): el XSD permite que el bloque no exista (minOccurs=0).
