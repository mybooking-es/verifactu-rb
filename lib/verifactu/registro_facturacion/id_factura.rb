  ...
  module RegistroFacturacion
    ...
    class IDFactura
      ...
      def initialize(id_emisor_factura:, num_serie_factura:, fecha_expedicion_factura:)
        ...
        caracteres_no = ['"', "'", '<', '>', '=']        
        raise Verifactu::VerifactuError, "num_serie_factura no puede contener los caracteres: #{caracteres_no.join(", ")}" if num_serie_factura.chars.any? { |char| caracteres_no.include?(char) }
        ...
        raise Verifactu::VerifactuError, "fecha_expedicion_factura no debe ser inferior a 28/10/2024" if Date.strptime(fecha_expedicion_factura, '%d-%m-%Y') < Date.new(2024, 10, 28)
